visualise_cascade3 <- function(data, pop_label = 1,
                              plot_label = ' ', 
                              plot_title = ' ', #y_axis = 'both',
                              colour_scheme = -1, second_colour = 'grey57',
                              order_reasons = 'auto',
                              space_y_PC =66.67, #but needs replacing
                              #buffer_x = 0, buffer_y = 0,
                              #reason_descr = 0,
                              #descr_prop = 0.4,
                              #label_buffer = -1,
                              font_size1= 4.0, font_size2 = 3.4,
                              verbose = F){
  ...
  p1 <- ggplot() + #geom_hline(yintercept = mx,
    ...
    geom_rect(data = data[data$in_Q==1,], 
                        aes(xmin = level - 0.5, 
                            xmax = level + 0.5,
                            ymin = 0, ymax = mx),#mx*(1+space_y_PC/100)), 
  ...
  for(i in 2:l){
    ...
    for(j in 1:no){
      ...
      txt_loc_x <- (x1 + x2)/2
      txt_loc_y <- 1#y2 + 5 # will need improving!!! #mx*( 1 + 0.5*space_y_PC/100 )
      ...
      N_pos_y <- base + mx/50#(aux[aux$order==j,]$N) + mx/50 #i.e. raise by a proportion of max N
  ...
  p2 <- p1 + geom_rect(data = dx, aes(xmin = sx1, xmax = sx2, ymin = sy1, ymax = sy2),
    ...
    geom_text(color = 'grey82', data = dx,
              aes(x = N_pos_x, y = N_pos_y, label = N_lab),
              vjust = 0, size = size2)

   #return(p2)
  
  ## Now separate plot for label action.
  p3 <- ggplot(data = dx) + geom_text(angle = 90, aes(x = text_loc_x, y = 45, label = text),
                 vjust = 0, hjust = 0.5, size = size2, color = 'grey19') +
    theme_classic() +
    theme(axis.ticks = element_blank(), 
         ...
         legend.position = 'none') + 
    xlab('') + ylab('a') + ylim(c(0,100)) +
    geom_rect(data = data[data$in_Q==1,], 
              aes(xmin = level - 0.5, xmax = level + 0.5,
                  ymin = 0, ymax =  90), color = 'grey73', fill = NA) +
  ##then top labels, with y-95
    geom_text(data = data[data$in_Q==1,],
        aes(y = 95, #0.1*(data[data$level==1,]$N),
          x = level,
          label = paste0(reason, ' (n=',N,')')),
  ...
  p4 <- cowplot::plot_grid(p3,p2,nrow=2, rel_heights = c(space_y_PC/100,1))
  return(p4)
}
# Are multiple y-axes here too fiddly? 
# I think you need a better theme- otherwise the spacing won't align
# make labels white in the upper panel??
